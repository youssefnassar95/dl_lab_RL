...
class ReplayBuffer:
    ...
    def __init__(self, capacity):
        ...
        self._data = self._data(states=[], actions=[],
    ...
    def add_transition(self, state, action, next_state, reward, done):
        ...
        if (self.memcount > self.capacity):
    ...
    def next_batch(self, batch_size):
        ...
        batch_actions = np.array([self._data.actions[i]
                                  for i in batch_indices])
        batch_next_states = np.array(
            [self._data.next_states[i] for i in batch_indices])
        batch_rewards = np.array([self._data.rewards[i]
                                  for i in batch_indices])
